###############################################################################
# This script runs meta-analysis using NIMARE. It does it based on the        #
# analysis method each paper employed (GLM or connectivity)                   #
#                                                                             #
...
for mri_col in mri:
    for beh_col in beh:
        key = f"{mri_col}-{beh_col}-{analysis}"
        temp_df = df[(df[mri_col] == 1) & (df[beh_col] == 1)]
        temp_df = temp_df[temp_df['analysis type']==analysis]
        if len(temp_df)>0:
